...
export class ClientesService {
  ...
  constructor(private db: AngularFirestore) {
    this.clientesColeccion = this.db.collection('clientes', ref => ref.orderBy('nombre', 'asc').where('pais', '==', 'Peru'));
  }
  ...
  editarCliente( c: Ecliente) {
    // idx: string,   this.clienteDoc = this.db.doc(`clientes/${idx}`);
